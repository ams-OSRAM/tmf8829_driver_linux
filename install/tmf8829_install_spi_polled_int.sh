#!/bin/bash
#
#
# make all the following files are in the current directory
# tmf8829.ko tmf8829-overlay-fpc-spi-polled.dtbo tmf8829_application.hex
# this script run with su privilege.
set -e

if grep -Fxq "dtoverlay=i2c0,pins_28_29" /boot/firmware/config.txt
then
    echo "1. Uncomment dtoverlay=i2c0,pins_28_29"
    sed -i 's/dtoverlay=i2c0,pins_28_29/#dtoverlay=i2c0,pins_28_29/' /boot/firmware/config.txt
else
    echo "1. No need to modify /boot/firmware/config.txt"
fi

if grep -Fxq "dtparam=spi=on" /boot/firmware/config.txt
then
    echo "2. No need to modify /boot/firmware/config.txt"
else
    echo "2. Append dtparam=spi=on to /boot/firmware/config.txt"
    echo "dtparam=spi=on" >> /boot/firmware/config.txt
fi

if grep -Fxq "dtoverlay=tmf8829-overlay-fpc-spi-polled" /boot/firmware/config.txt
then
    echo "3. No need to modify /boot/firmware/config.txt"
else
    echo "3. Append dtoverlay=tmf8829-overlay-fpc-spi-polled to /boot/firmware/config.txt"
    echo "dtoverlay=tmf8829-overlay-fpc-spi-polled" >> /boot/firmware/config.txt
fi

cp tmf8829-overlay-fpc-spi-polled.dtbo /boot/overlays/
cp tmf8829_application.hex /lib/firmware/tmf8829_application.hex
cp spi/tmf8829.ko /opt/USBSensorBridgeRuntime/modules/
cp ams-usb-sensorbridge.service /etc/systemd/system
cp tmf8829_zmq_server.service /etc/systemd/system/
