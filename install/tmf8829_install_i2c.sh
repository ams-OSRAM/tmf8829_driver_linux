...
# tmf8829.ko tmf8829-overlay-fpc.dtbo tmf8829_application.hex
...
if grep -Fxq "dtparam=i2c0_baudrate=400000" /boot/firmware/config.txt
then
    echo "2. No need to modify /boot/firmware/config.txt"
else
    echo "2. Append dtparam=i2c0_baudrate=400000 to /boot/firmware/config.txt"
    echo "dtparam=i2c0_baudrate=400000" >> /boot/firmware/config.txt
fi

if grep -Fxq "dtoverlay=tmf8829-overlay-fpc" /boot/firmware/config.txt
then
    echo "3. No need to modify /boot/firmware/config.txt"
else
    echo "3. Append dtoverlay=tmf8829-overlay-fpc to /boot/firmware/config.txt"
    echo "dtoverlay=tmf8829-overlay-fpc" >> /boot/firmware/config.txt
fi

cp tmf8829-overlay-fpc.dtbo /boot/overlays/
