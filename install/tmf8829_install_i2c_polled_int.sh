...
# tmf8829.ko tmf8829-overlay-fpc-polled.dtbo tmf8829_application.hex
...
if grep -Fxq "dtoverlay=tmf8829-overlay-fpc-polled" /boot/firmware/config.txt
then
    echo "3. No need to modify /boot/firmware/config.txt"
else
    echo "3. Append dtoverlay=tmf8829-overlay-fpc-polled to /boot/firmware/config.txt"
    echo "dtoverlay=tmf8829-overlay-fpc-polled" >> /boot/firmware/config.txt
fi
